...
def select_portfolio(tickers, window=5):
    ...
    try:
        ...
        for ticker in tickers:
            resp = binance.fetch_ohlcv(ticker, '1d', limit=window)
            df = DataFrame(resp, columns=['date', 'open', 'high', 'low', 'close', 'volume'])
...
def cal_moving_average(ticker="BTC", window=5):
    ...
    '''
    try:
        resp = binance.fetch_ohlcv(ticker, '1d', limit=5)
        df = DataFrame(resp, columns=['date', 'open', 'high', 'low', 'close', 'volume'])
        close = df['close']
    ...
    try:
        for ticker in portfolio:
            ...
            if price >= target and high <= target * 1.02  and price >= ma5 and holdings[ticker] is False:
                ...
                resp = binance.fetch_tickers(ticker)
                sell_price = resp[ticker]['ask']
                unit = budget_per_coin/float(sell_price)

                if DEBUG is False:
                    binance.create_market_buy_order(ticker, unit)
                else:
...
def cal_budget():
    ...
    '''
    try:
        usdt_balance = binance.fetch_free_balance()['USDT']
...
def print_status(tickers, now, prices, targets, high_prices):
    '''
    코인별 현재 상태를 출력
    :param tickers: 티커 리스트
    :param prices: 가격 리스트
    ...
    try:
        ...
        print(now)
        for ticker in tickers:
            print("{:10} 목표가: {:>4.4f} 현재가: {:>4.4f}  고가: {:>8.4f}".format(ticker, targets[ticker], prices[ticker], high_prices[ticker]))
    except:
        pass
...
response = binance.fetch_markets()                                      # USDT 마켓의 티커 리스트 얻기
tickers = [x['symbol'] for x in response if x['id'].endswith(MARKET)]
...
while True:
    ...
    if setup_time1 < now < setup_time2:
        response = binance.fetch_markets()                                  # USDT 마켓의 티커 목록 갱신
        tickers = [x['symbol'] for x in response if x['id'].endswith(MARKET)]
